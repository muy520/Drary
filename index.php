		...
		<article class="index-post-article">
			<div class="index-post-img">
				...
				<a href="<?php $this->permalink() ?>">
					<?php if (array_key_exists('img',unserialize($this->___fields()))): ?><?php _e('<img class="index-text-img" alt="background" data-src="') ?><?php $this->fields->img(); ?><?php _e('">') ?><?php else: ?><?php preg_match_all("/\<img.*?src\=(\'|\")(.*?)(\'|\")[^>]*>/i", $this->content, $matches);$imgCount = count($matches[0]);if($imgCount >= 1){$img = $matches[2][0];_e('<img class="index-text-img" alt="background"  data-src="'.$img.'">');}else {_e('<img class="index-text-img" alt="background" data-src="/usr/themes/Diary/img/'.rand(1,10).'.png">');} ?><?php endif; ?>
		        	<div class="text-img">
